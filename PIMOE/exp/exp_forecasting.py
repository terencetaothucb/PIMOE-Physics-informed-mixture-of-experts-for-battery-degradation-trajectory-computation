...
class Exp_Long_Term_Forecast1(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        if test:
            print('loading model')
            self.model.load_state_dict(torch.load(os.path.join('checkpoints/PIMOE/NCA/CY45-05_1/PIMOE_dsNCA_ex5_pl50_tk2_dm0/checkpoint.pth')))
        self.model.eval()
        ...
        with torch.no_grad():
            for X_batch, y_batch in data_loader:
                ...
                rmse = np.sqrt(np.mean((y_pred_inv - y_batch_inv)**2))
                mape = np.mean(np.abs((y_pred_inv - y_batch_inv) / y_batch_inv)) * 100
